...
import MysteryBoxUI from "@/components/MysteryBoxUI";
...
const STATUS_META: Record<string, { label: string; color: string; desc: string }> = {
  ...
  REFUNDED: { label: "Refunded", color: "text-royal-muted border-royal-border bg-royal-card", desc: "Order refunded" },
  CANCELLED: { label: "Cancelled", color: "text-royal-muted border-royal-border bg-royal-card", desc: "Order was cancelled" },
};

function formatOrderAmount(order: Pick<OrderInfo, "amountUsd" | "amountKhr" | "currency">): string {
  if (order.currency === "KHR" && typeof order.amountKhr === "number" && Number.isFinite(order.amountKhr)) {
    return `${Math.round(order.amountKhr ?? 0).toLocaleString("en-US")} ៛`;
  }
  return `$${order.amountUsd.toFixed(2)}`;
}

function formatSecondaryAmount(order: Pick<OrderInfo, "amountUsd" | "amountKhr" | "currency">): string | null {
  if (order.currency === "KHR" && typeof order.amountKhr === "number" && Number.isFinite(order.amountKhr)) {
    ...
    <>
      ...
      <main className="mx-auto max-w-2xl px-4 py-12 sm:py-16 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-10">
          ...
          <p className="text-royal-muted text-sm sm:text-base">Enter your order number to see the latest status.</p>
        </div>
        ...
        {order && meta && (
          <div className="card overflow-hidden fade-up">
            <div className="p-5 sm:p-6 border-b border-royal-border">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-royal-muted mb-1">Order</p>
              ...
              {isPolling && (
                <div className="flex items-center gap-2 mt-2 text-xs text-royal-muted">
            ...
            {order.isMysteryBox && (
              <div className="px-5 sm:px-6 pb-6">
                <MysteryBoxUI orderNumber={order.orderNumber} initialStatus={order.status} />
              </div>
            )}

            {!isFailed && (
              <div className="px-5 sm:px-6 py-6 border-b border-royal-border">
                      ...
                      <div key={step.key} className="flex flex-1 items-center">
                        <div className="flex flex-col items-center gap-2">
                          <div
                            className={`relative flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-full border-2 transition-all duration-700 ${
                              isReached
                                ...
                                : "border-royal-border bg-royal-card text-royal-muted"
                            }`}
                          >
                            {isReached ? (
                          ...
                          <span
                            className={`text-[10px] sm:text-xs font-medium text-center ${
                              isReached ? "text-green-400" : "text-royal-muted"
                            }`}
                          >
                            {step.label}
                          </span>
                        ...
                        {i < TIMELINE_STEPS.length - 1 && (
                          <div className="flex-1 mx-2 sm:mx-3">
                            <div className="relative h-1 rounded-full bg-royal-border overflow-hidden">
                              <div
            ...
            {isFailed && (
              <div className="px-5 sm:px-6 py-5 border-b border-royal-border">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-royal-muted">{meta.desc}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="p-5 sm:p-6 space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-royal-muted">Player UID</span>
              ...
              <div className="flex justify-between">
                <span className="text-royal-muted">Amount</span>
                <div className="text-right">
                  <div className="font-bold text-royal-primary">{formatOrderAmount(order)}</div>
                  {secondaryAmount && (
                    <div className="text-xs text-royal-muted">{secondaryAmount}</div>
                  )}
                </div>
              </div>
              <div className="border-t border-royal-border pt-3 space-y-3">
                ...
                {order.paidAt && (
                  <div className="flex justify-between text-xs">
                    <span className="text-royal-muted">Paid</span>
                    <span>{new Date(order.paidAt).toLocaleString()}</span>
                  </div>
                )}
                {order.deliveredAt && (
                  <div className="flex justify-between text-xs">
                    <span className="text-royal-muted">Delivered</span>
            ...
            {order.status === "DELIVERED" && (
              <div className="mx-5 sm:mx-6 mb-5 sm:mb-6 space-y-3">
                ...
                <a
                  href={`https://t.me/share/url?url=${encodeURIComponent(typeof window !== "undefined" ? window.location.href : "")}&text=${encodeURIComponent(`⚡ Just topped up ${order.productName} for ${order.gameName} on Ty Khai TopUp! Fast & easy 🔥`)}`}
                  target="_blank"
            ...
            {["PAID", "PROCESSING", "DELIVERED"].includes(order.status) && (
              <div className="mx-5 sm:mx-6 mb-5 sm:mb-6">
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full rounded-xl border border-royal-primary/40 bg-royal-primary/10 hover:bg-royal-primary/20 text-royal-primary py-3 text-sm font-medium transition-colors"
                >
